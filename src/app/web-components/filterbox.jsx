...
export default function FilterBox({basedata}) {
    ...
    <div className="flex flex-row m-4 sticky top-2 z-10 w-9">
        ...
        <div className="dropdown">
            ...
            <CheckboxTree dict={basedata.meta.locations} onChange={setActiveLocations}  />
            </div>
        </div>
        <label className="input rounded-field bg-base-200 border border-base-300 grow w-56">
            ...
            <input id="search" type="search" className="grow w-48" placeholder="Search" onChange={(event)=>setSearchTerm(event.target.value)} />
        </label>
        <div className="join rounded-field flex-none ml-4">
            ...
            <Link href={{pathname:"/"}} onClick={resetFields}><button className="btn join-item btn-primary">Reset</button></Link>
        </div>
    </div>
    )};